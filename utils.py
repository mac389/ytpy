...
def bigrams(comments,drug):
	bi = BigramAssocMeasures()
	bi_finder = BigramCollocationFinder.from_words(Text(flatten(comments)), window_size=20)
	top_general = bi_finder.nbest(bi.pmi,20)
...
def LSA(texts, threshold=5):
	'''Standard pre-processing
		1. Remove stopwords
		2. Remove words that occur less than threshold times
		'''
	#texts = map(remove_stopwords,texts) <- Susan Dumais @ MSFT on Scholarpedia says don't so this
...
def plot_word_frequency(freqDist,cutoff=75,normalized=True):
	...
	line, = ax.plot(vals[:cutoff], linewidth=2)	
	line.set_clip_on(False)
	
	ax.set_ylabel('Frequency')
	
	ax.set_xticks(arange(cutoff))
	ax.set_xticklabels(labels[:cutoff], rotation=90)
